...
Layer 7 as its for communications and it serves as an medium for communication 
...
<br>
 ...
 UDP he needs speed over accuracy
 - What are some cons of this protocol? Give an example of a potential issue.
Sometimes this protocol sacrifices accuracy for speed. Transferring files between apps or devices

2. TCP has error checking, which ensures that all packets arrive properly. Why is this important?
To make sure all the packets are recieved in the right order ensuring the correct data
<br>
 - Give an example of how TCP ensures that there are no errors.
It keeps track of which packets have been received successfully, resending any that have been lost or damaged, and specifying the order for reassembling the data on the other end
...
<br>
 ...
 TCP: B sends ACK to A for recieved packets to tell A it got the packets
 UDP: There is no built-in mechanism to guarantee acknowledgment of packet delivery.

 - What is another use of this?
Utilizing error checking and correction is beneficial in this context because TCP enables a server to request retransmission, thereby enhancing the integrity of communication.
